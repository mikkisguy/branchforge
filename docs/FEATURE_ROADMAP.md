...
> **Note**: Section 17 (label_characters junction table) was removed. Characters are now derived from `label_lines.speakerId` via a read-only association. See the Labels section (14) and the GET `labels/:id/characters` endpoint (labels read-only endpoint) for details.
