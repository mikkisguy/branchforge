 ...
 *
 * Dialog for managing state variables for a project.
 * State variables are boolean state variables used in conditional branching.
 */

import { useState, useCallback, useEffect, useRef, useMemo } from "react";
import { Loader2, X, Plus, Trash2, Pencil } from "lucide-react";
...
interface StateVariablesDialogProps {
  open: boolean;
...
export function StateVariablesDialog({
  open,
  onOpenChange,
  projectId,
}: StateVariablesDialogProps) {
  const {
    stateVariables,
  ...
  useEffect(() => {
    ...
    if (open && stateVariables.length > 0) {
    ...
    } else if (open && stateVariables.length === 0) {
      // Initialize with empty state variables
      setStateVariablesList([]);
      hasInitialized.current = true;
    }
  }, [open, stateVariables, isSaving]);
