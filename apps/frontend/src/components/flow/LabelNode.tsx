...
// ─── Tooltip portal (lazy-mounted — only exists while a node is hovered) ───

/**
 * Portal-rendered hover tooltip. This component only mounts when the parent
 * node is hovered, so its hooks (useState ×3, useCallback, useEffect ×2) do
 ...
 */
function LabelNodeTooltipPortal({
  data,
  nodeRef,
}: {
  data: LabelNodeData;
...
// ─── Node component (lightweight — tooltip deferred to hover) ──────────────
