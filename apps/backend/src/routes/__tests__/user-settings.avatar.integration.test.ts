 ...
 *
 * Tests for the user avatar upload functionality against a real database.
 * These tests verify:
...
import { userSettingsAvatarRoutes } from "../user-settings.routes.js";
...
const otherUser: NewUser = {
  id: otherUserId,
  email: testEmail("user-settings-avatar", "other"),
  passwordHash: "hashed_password",
  role: "OWNER",
};
...
describe("User Settings Avatar Routes (Integration)", () => {
  ...
  async function cleanupTestData() {
    ...
    await db.delete(userSettings).where(eq(userSettings.userId, otherUserId));
  ...
  beforeEach(async () => {
    ...
    await fastify.register(session, {
      secret: "test-session-secret-for-user-settings-avatar-tests",
    ...
    // Register the routes
    await fastify.register(userSettingsAvatarRoutes, { prefix: "/api" });
  ...
  describe("POST /user/settings/avatar", () => {
  ...
  describe("DELETE /user/settings/avatar", () => {
    it("should delete an existing avatar", async () => {
      ...
      expect(settings.avatarUrl).toBeNull();
    ...
    it("should succeed even when no avatar exists", async () => {
      ...
      // Verify avatarUrl is still null in database
      const [settings] = await db
        .select()
        .from(userSettings)
        .where(eq(userSettings.userId, testUserId))
