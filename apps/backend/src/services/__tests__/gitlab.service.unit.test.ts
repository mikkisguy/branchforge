...
import { gitlabIntegrations, gitlabRepositories } from '../../db/schema/index.js';
...
describe('GitLabService', () => {
  ...
  describe('validateGitlabPAT', () => {
    it('should return username for valid PAT', async () => {
      vi.mocked(encryptionService.validateAndGetUsername).mockResolvedValue('testuser');

      const result = await validateGitlabPAT(testToken);

      expect(result).toBe('testuser');
      expect(encryptionService.validateAndGetUsername).toHaveBeenCalledWith(testToken, 'https://gitlab.com');
    });

    it('should work with custom GitLab URL', async () => {
      vi.mocked(encryptionService.validateAndGetUsername).mockResolvedValue('customuser');

      const result = await validateGitlabPAT(testToken, testGitlabUrl);

      expect(result).toBe('customuser');
      expect(encryptionService.validateAndGetUsername).toHaveBeenCalledWith(testToken, testGitlabUrl);
    });

    it('should return null for invalid PAT', async () => {
      vi.mocked(encryptionService.validateAndGetUsername).mockResolvedValue(null);
  ...
  describe('storeGitlabIntegration', () => {
    it('should store new integration', async () => {
      vi.mocked(encryptionService.validateAndGetUsername).mockResolvedValue('testuser');
      vi.mocked(encryptionService.encryptPAT).mockReturnValue('encrypted_test_token');
    ...
    it('should throw when token validation fails', async () => {
      ...
      await expect(storeGitlabIntegration(testUserId, testToken, testGitlabUrl)).rejects.toThrow('Invalid GitLab token');
    });
  });
  ...
  describe('listGitlabProjects', () => {
    it('should list user projects', async () => {
      ...
      const result = await listGitlabProjects(testUserId, testGitlabUrl);
    ...
    it('should handle pagination', async () => {
      ...
      const page1Projects = [
        { id: 1, name: 'project1', path_with_namespace: 'user/project1', default_branch: 'main' },
      ];
      const page2Projects = [
        { id: 2, name: 'project2', path_with_namespace: 'user/project2', default_branch: 'main' },
      ];
      ...
      nock(testGitlabUrl)
        ...
        .matchHeader('private-token', testToken)
        .query({ membership: 'true', per_page: '100', page: '1' })
        .reply(200, page1Projects, { 'x-total-pages': '2', 'x-next-page': '2' });
      ...
      expect(result).toEqual([...page1Projects, ...page2Projects]);
    });
    ...
    it('should throw when integration not found', async () => {
      ...
      await expect(listGitlabProjects(testUserId, testGitlabUrl)).rejects.toThrow('GitLab integration not found');
    });
  });
  ...
  describe('linkRepository', () => {
    it('should link project to repository', async () => {
      ...
      mockDb.insert.mockReturnValue({ values: vi.fn().mockReturnValue(mockInsert) });

      await linkRepository(testProjectId, testGitlabProjectId, testRepositoryName, testBranch);
    ...
    it('should use default branch when not provided', async () => {
      ...
      await linkRepository(testProjectId, testGitlabProjectId, testRepositoryName);
  ...
  describe('listBranches', () => {
    ...
    it('should throw when repository not linked', async () => {
      ...
      await expect(listBranches(testProjectId, testGitlabUrl)).rejects.toThrow('GitLab repository not linked');
    });
  });
  ...
  describe('listRpyFiles', () => {
    it('should list .rpy files in repository', async () => {
      ...
      nock(testGitlabUrl)
        ...
        .matchHeader('private-token', testToken)
        .query(actualQuery => {
          return actualQuery.ref === testBranch &&
                 actualQuery.recursive === 'true' &&
                 actualQuery.per_page === '100';
        })
        .reply(200, mockTree, { 'x-total-pages': '1' });

      const result = await listRpyFiles(testProjectId, testBranch, testGitlabUrl);
  ...
  describe('getFileContent', () => {
    it('should get file content from repository', async () => {
      ...
      nock(testGitlabUrl)
        .get(`/api/v4/projects/${testGitlabProjectId}/repository/files/game%2Fscript.rpy`)
        .matchHeader('private-token', testToken)
      ...
      const result = await getFileContent(testProjectId, 'game/script.rpy', testBranch, testGitlabUrl);
    ...
    it('should return null for non-existent file', async () => {
      ...
      nock(testGitlabUrl)
        .get(`/api/v4/projects/${testGitlabProjectId}/repository/files/game%2Fmissing.rpy`)
        .matchHeader('private-token', testToken)
        .query({ ref: testBranch })
        .reply(404);

      const result = await getFileContent(testProjectId, 'game/missing.rpy', testBranch, testGitlabUrl);
  ...
  describe('createOrUpdateFile', () => {
    it('should create new file in repository', async () => {
      ...
      nock(testGitlabUrl)
        .put(`/api/v4/projects/${testGitlabProjectId}/repository/files/game%2Fnew.rpy`)
        .matchHeader('private-token', testToken)
        .reply(200, {
    ...
    it('should update existing file in repository', async () => {
      ...
      nock(testGitlabUrl)
        .put(`/api/v4/projects/${testGitlabProjectId}/repository/files/game%2Fscript.rpy`)
        .matchHeader('private-token', testToken)
        .reply(200, {
          file_path: 'game/script.rpy',
          branch: testBranch,
    ...
    it('should throw on API error', async () => {
      ...
      await expect(
        createOrUpdateFile(testProjectId, testBranch, 'game/script.rpy', 'content', 'message', testGitlabUrl)
      ).rejects.toThrow();
    });
  });
});
