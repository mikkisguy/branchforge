...
function detectIndentUnit(lines: string[]): number {
  ...
  for (const line of lines) {
    ...
    if (leadingWhitespace.includes('\t')) {
      ...
      // Spaces detected
      indentCounts.set(leadingWhitespace.length, (indentCounts.get(leadingWhitespace.length) || 0) + 1);
    }
  }
  ...
  const indents = Array.from(indentCounts.keys()).filter((n) => n > 0).sort((a, b) => a - b);
  ...
  for (const [labelTitle, labelData] of labelMap) {
    if (labelData?.effects && !effectsInserted.has(labelTitle)) {
      const hasEffects =
        ...
        (labelData.effects.stateVariablesUnset &&
          labelData.effects.stateVariablesUnset.length > 0) ||
        (labelData.effects.meters && Object.keys(labelData.effects.meters).length > 0);
      ...
      if (hasEffects) {
        ...
        for (let i = 0; i < result.length; i++) {
          const match = result[i].match(/^(\s*)label\s+([a-zA-Z_][a-zA-Z0-9_]*)/);
...
export function generateStateVariablesFile(
  stateVariables: Array<{ key: string; description?: string | null; category?: string | null }>
): string {
  const lines: string[] = [];
