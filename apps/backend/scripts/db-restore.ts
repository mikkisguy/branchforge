  ...
  for (let i = 0; i < content.length; i++) {
    ...
    if (inLineComment && (char === "\n" || char === "\r")) {
      ...
      if (
        ...
        !currentStatement.trim().startsWith("--")
      ) {
    ...
    if (inDollarQuote && char === "$") {
      ...
      if (
        possibleEnd >= 0 &&
        content.substring(possibleEnd, i + 1) === dollarQuoteTag
      ) {
        inDollarQuote = false;
        dollarQuoteTag = "";
        currentStatement += char;
...
async function restoreBackup() {
  ...
  try {
    ...
    console.log(
      `   Server: ${dbInfo.rows[0].inet_server_addr}:${dbInfo.rows[0].inet_server_port}`
    );
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      if (stmt && !stmt.startsWith("--")) {
        ...
        try {
          ...
          console.error(
            `   ❌ Error executing statement ${i + 1}: ${stmtType}`
          );
    ...
    `);
    console.log(
      `   admin_settings: ${verification.rows[0].admin_settings} rows`
    );
    console.log(
      `   gitlab_integrations: ${verification.rows[0].gitlab_integrations} rows`
    );
