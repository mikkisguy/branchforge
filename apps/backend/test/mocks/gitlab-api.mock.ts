 ...
 */
export function mockInvalidToken(token: string = testFixtures.gitlab.invalidToken) {
  return nock(GITLAB_API_BASE)
    .get('/user')
    .matchHeader('private-token', token)
    .reply(401, {
...
export function mockPaginatedProjectsList(
  ...
  const scope = nock(GITLAB_API_BASE)
    .persist()
    .get('/projects')
    .matchHeader('private-token', token);
...
export function mockBranchesList(
  ...
  return nock(GITLAB_API_BASE)
    ...
    .matchHeader('private-token', token)
    .reply(200, branches.map(name => ({ name, commit: { id: 'abc123' } })));
}
...
export function mockRepositoryTree(
  ...
  return nock(GITLAB_API_BASE)
    ...
    .matchHeader('private-token', token)
    .query({ ref: branch, recursive: 'false' })
    .reply(200, files);
}
...
export function mockGetFile(
  ...
  return nock(GITLAB_API_BASE)
    .get(`/projects/${projectId}/repository/files/${encodeURIComponent(filePath)}`)
    .matchHeader('private-token', token)
    .query({ ref: branch })
    .reply(200, {
...
export function mockCreateOrUpdateFile(
  ...
  return nock(GITLAB_API_BASE)
    .put(`/projects/${projectId}/repository/files/${encodeURIComponent(filePath)}`)
    .matchHeader('private-token', token)
    .reply(200, {
      file_path: filePath,
      branch: branch,
